...
@Service
public class JaCoPScheduleGenerator implements ScheduleGenerator {
	 ...
	 *
	 * \(available_{i,j}\) is 1 if employee \(i\) is available on slot \(j\) else
	 * 0.<br>
	 * 
	 ...
	 * \(minHours_i\) is the minimum number of hours that employee \(i\) will
	 * work.<br>
	 * \(maxHours_i\) is the maximum number of hours that employee \(i\) will
	 * work.<br>
	 * 
	 ...
	 * 
	 * 
	 * \(days_k\) is the number of sub sections that we are splitting our schedule
	 * in<br>
	 * 
	 * 
	 * 
	 ...
	 * <br>
	 * <b>Constraints:</b><br>
	 * If the employee is not available for the time slot then the employee cannot
	 * work the time slot.<br>
	 * \(\forall_{i}\forall_{j}(\neg available_{i,j} \implies work_{i,j} = 0)\) <br>
	 * 
	 * 
	 * Must have no less than minimum number of employees and no more than maximum
	 * number of employees for slot.<br>
	 ...
	 * 
	 * There must be at least one viable pair that is working, additionally
	 * constrains that there must be at least 1 food maker and at least 1 drink
	 * maker.<br>
	 * \(\forall_j\forall_{(i_1,i_2)}\)<br>
	 * \((i_2 &gt; i_1 \wedge (food_{i_1} + drink_{i_2}\ge 2\vee food_{i_2} +
	 * drink_{i_1}\ge 2)\implies work_{i_1,j}\cdot work_{i_2,j}\ge 2)\)<br>
	 * 
	 * 
	 * 
	 * 
	 * For each employee they can work no less than their minimum and no more than
	 * their maximum number of hours.<br>
	 * \(\forall_{i}(minHours_i \le \sum\limits_{j}time_j\cdot work_{i,j}\le
	 * maxHours_i)\)<br>
	 * 
	 * If the time slot is admin only then if an employee is not an admin then they
	 * are not available for that shift.<br>
	 ...
	 * <br>
	 * <b>Output:</b><br>
	 * \(\forall_i\forall_j(work_{i,j}\text{ is assigned})\)<br>
	 * 
	 * @param available
	 *            is 1 if employee \(i\) is available on slot \(j\) else 0.
	 * @param admin
	 *            is 1 if employee \(i\) is an admin else 0.
	 * @param canLift
	 *            is 1 if employee \(i\) can lift heavy tea container else 0.
	 * @param food
	 *            is 1 if employee \(i\) can make food else 0.
	 * @param drink
	 *            is 1 if employee \(i\) can make drinks else 0.
	 * @param minHours
	...
	public ScheduleResult schedule(String available, String admin, String canLift, String food, String drink,
			String minHours, String maxHours, String employeeCount, String time, String adminOnly, String days) {
		return schedule(matrixUtil.convertMatrix(available), matrixUtil.convert(admin), matrixUtil.convert(canLift),
				matrixUtil.convert(food), matrixUtil.convert(drink), matrixUtil.convert(minHours),
				matrixUtil.convert(maxHours), matrixUtil.convert(employeeCount), matrixUtil.convert(time),
				matrixUtil.convert(adminOnly), matrixUtil.convert(days));
	}
	 ...
	 *            is 1 if employee \(i\) is available on slot \(j\) else 0.
	 * @param admin
	 *            \\(admin_i\\) is 1 if employee \(i\) is an admin else 0.
	 * @param canLift
	 *            is 1 if employee \(i\) can lift heavy tea container else 0.
	 * @param food
	 *            is 1 if employee \(i\) can make food else 0.
	 ...
	 * @return the schedule
	 */
	private ScheduleResult schedule(int[][] available, int[] admin, int[] canLift, int[] food, int[] drink,
			int[] minHours, int[] maxHours, int[] employeeCount, int[] time, int[] adminOnly, int[] days) {
		...
		for (int i = 0; i < numberOfEmployees; i++)
			for (int k = 0; k < days.length; k++)
				store.impose(
						new SumBool(store, Arrays.copyOfRange(work[i], k == 0 ? 0 : days[k - 1], days[k]), "<=", ONE));

		// compute oldSchedule
		Search<IntVar> label = new DepthFirstSearch<IntVar>();
		// label.setPrintInfo(false);

		SelectChoicePoint<IntVar> select = new SimpleMatrixSelect<>(work, new IndomainMin<IntVar>());
